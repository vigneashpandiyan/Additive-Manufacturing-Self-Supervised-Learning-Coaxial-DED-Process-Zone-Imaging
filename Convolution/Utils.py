...
def plot_curves(train_losses,val_losses,Accuracy):
    
    plt.rcParams.update(plt.rcParamsDefault)
    fig = plt.figure(figsize=(6,4), dpi=100)
    plt.xlabel('Epoch')
    plt.ylabel('Loss values')
    plt.plot(train_losses,'red', marker='o',label='Train Loss',linewidth =2.0)
    plt.plot(val_losses, 'blue',marker='*',label="Validation Loss",linewidth =2.0)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.plot(Accuracy,'red', marker='o',label='Accuracy',linewidth =2.0)
    #plt.plot(val_losses, label="Validation Loss")
    plt.legend( loc='lower right')
    plt.savefig('Accuracy_Conv.png', dpi=600,bbox_inches='tight')
    plt.show()
