...
def Three_embeddings(embeddings, targets,graph_name,ang, xlim=None, ylim=None):
    ...
    for i in uniq:
        ...
        b=x2[indx]
        c=x3[indx]
        ax.plot(a, b, c ,color=color[j],label=uniq[j],marker=marker[j],linestyle='',ms=16)
        j=j+1
     
    plt.xlabel ('Dimension-1', labelpad=15,fontsize = 25)
    plt.ylabel ('Dimension-2', labelpad=15,fontsize = 25)
    ax.set_zlabel('Dimension-3',labelpad=15,fontsize = 25)
...
def Three_Latent_embeddings(embeddings, targets,graph_name,ang, dim_1, dim_2, dim_3,xlim=None, ylim=None):
    ...
    plt.title(str(graph_title),fontsize = 25)
    plt.legend(loc='upper left',frameon=False)
    plt.savefig(os.path.join(folder, graph_name), bbox_inches='tight',dpi=400)
    plt.show()
    return ax,fig
